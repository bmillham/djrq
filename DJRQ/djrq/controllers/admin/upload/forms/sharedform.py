...
class SharedForm(web.core.HTTPMethod):
    def __before__(self, *args, **kw):
        options = session.query(SiteOptions).one()
        kw['options'] = options
        kw['limit_requests'] = options.limit_requests
        return super(SharedForm, self).__before__(*args, **kw)

    def __after__(self, result, *args, **kw):
        kw.update(result)
        return super(SharedForm, self).__after__(('djrq.templates.admin.upload.forms.sharedform', kw))
